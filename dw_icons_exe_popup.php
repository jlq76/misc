...
$tmp = array();
// -----------------------------------------------------------------------------
// FIX #1: replace obsolete trigger_event()
// -----------------------------------------------------------------------------
//trigger_event('MEDIAMANAGER_STARTED', $tmp);
use dokuwiki\Extension\Event;
if (class_exists('Event')) {
    Event::createAndTrigger('MEDIAMANAGER_STARTED', []);
}
...
?><!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="<?php echo $conf['lang'] ?>" lang="<?php echo $conf['lang'] ?>" dir="<?php echo $lang
['direction'] ?>" class="no-js">
<head>
  <meta charset="UTF-8" />
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  var is_bootstrap = (typeof window.jQuery !== "undefined" && window.jQuery.fn && window.jQuery.fn.modal);

  var icon_pack  = document.getElementById('icon_pack'),
      icon_name  = document.getElementById('icon_name'),
      icon_size  = document.getElementById('icon_size'),
      icon_align = document.getElementById('icon_align'),
  ...
  document.querySelectorAll('button[data-icon-size]').forEach(function(btn){
    btn.addEventListener('click', function(){
      document.querySelectorAll('button[data-icon-size]').forEach(function(b){ b.classList.remove('active'); });
      btn.classList.add('active');

      icon_size.value = btn.getAttribute('data-icon-size');

      var event = new Event('popup:build');
      document.dispatchEvent(event);
    });
  });

  document.querySelectorAll('button[data-icon-align]').forEach(function(btn){
    btn.addEventListener('click', function(){
      document.querySelectorAll('button[data-icon-align]').forEach(function(b){ b.classList.remove('active'); });
      btn.classList.add('active');

      icon_align.value = btn.getAttribute('data-icon-align');
  ...
  document.querySelectorAll('ul.nav a').forEach(function(link){
    link.addEventListener('click', function(e){
      ...
      if (!is_bootstrap) {
        document.querySelectorAll('.tab-pane').forEach(function(el){ el.style.display = 'none'; });
        document.querySelectorAll('ul.nav li.active').forEach(function(li){ li.classList.remove('active'); });
      ...
      icon_pack.value = link.getAttribute('data-pack');
      document.querySelectorAll('.preview-box').forEach(function(el){ el.classList.remove('hide'); });

      var event = new Event('popup:reset');
      document.dispatchEvent(event);
    });
  });
  ...
  document.querySelectorAll('.btn-icon').forEach(function(btn){
    btn.addEventListener('click', function(){
      icon_name.value = btn.getAttribute('data-icon-name');
  ...
  document.addEventListener('popup:build', function(){
    var pack  = icon_pack.value,
        size  = icon_size.value,
        align = icon_align.value,
    ...
    switch(size){
      case 'small': icon_size_pixel = 24; break;
      case 'medium': icon_size_pixel = 32; break;
      case 'large': icon_size_pixel = 48; break;
    }
    if (icon_size_pixel) syntax.push('?' + icon_size_pixel);
    if (align) syntax.push('&align=' + align);
    syntax.push('}}');

    output.value = syntax.join('');
    preview.textContent = syntax.join('');
  });
  ...
  document.addEventListener('popup:reset', function(){
    document.querySelectorAll('form').forEach(function(form){ form.reset(); });
    output.value = '';
    preview.textContent = '';
  });

  document.querySelectorAll('#btn-preview, #btn-insert').forEach(function(btn){
    btn.addEventListener('click', function(){
      if (btn.id === 'btn-insert') {
        opener.insertAtCarret('wiki__text', output.value);
        opener.focus();
      }
    });
  });
